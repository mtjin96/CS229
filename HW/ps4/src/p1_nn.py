...
def backward_softmax(x, grad_outputs):
    ...
    s = forward_softmax(x)
    jacob_mat = np.diag(s)
    for i in range(len(jacob_mat)):
        for j in range(len(jacob_mat)):
            if i == j:
                jacob_mat[i][j] = s[i] * (1 - s[j])
            else:
                jacob_mat[i][j] = -s[j] * s[i]
    grad = grad_outputs.dot(jacob_mat)
    #grad = jacob_mat * grad_outputs
    return grad
    # *** END CODE HERE ***
...
def backward_relu(x, grad_outputs):
    ...
    """
    # *** START CODE HERE ***
    grad = np.array(grad_outputs, copy=True)
    grad[x <= 0] = 0
    return grad
    # *** END CODE HERE ***
...
def backward_convolution(conv_W, conv_b, data, output_grad):
    ...
    # *** START CODE HERE ***
    # dimensions from data's shape
    input_channels, input_width, input_height = data.shape

    # dimensions from W's shape
    conv_channels, _, conv_width, conv_height = conv_W.shape

    # dimensions from output_grad's shape
    _, width, height = output_grad.shape

    # Initializing gradient
    grad_data = np.zeros_like(data)
    grad_weights = np.zeros_like(conv_W)
    grad_bias = np.zeros_like(conv_b)

    # gradient
    for w in range(width):
        for h in range(height):
            for channel in range(conv_channels):
                grad_weights[channel, :, :, :] += data[:, w:w + conv_width, h:h + conv_height] * output_grad[channel, w, h]
                grad_bias[channel] += output_grad[channel, w, h]
                grad_data[:, w:w + conv_width, h:h + conv_height] += conv_W[channel, :, :, :] * output_grad[channel, w, h]
...
def backward_max_pool(data, pool_width, pool_height, output_grad):
    ...
    H = input_height // pool_height
    W = input_width // pool_width
    ...
    for c in range(input_channels):
        for w in range(W):
            for h in range(H):
                h1 = h * stride_h
                h2 = h * stride_h + pool_height
                w1 = w * stride_w
                w2 = w * stride_w + pool_width
                window = data[c, w1:w2, h1:h2]
                window2 = np.reshape(window, (pool_height * pool_width))
                window3 = np.zeros_like(window2)
                window3[np.argmax(window2)] = 1
                grad_data[c, w1:w2, h1:h2] = np.reshape(window3, (pool_width, pool_height)) * output_grad[c, w, h]
...
def backward_cross_entropy_loss(probabilities, labels):
    ...
    # *** START CODE HERE ***
    grad = - np.divide(labels, probabilities)
    return grad
    # *** END CODE HERE ***
...
def backward_linear(weights, bias, data, output_grad):
    ...
    # *** START CODE HERE ***
    grad_weights = np.dot((np.array(data)[np.newaxis]).T, np.array(output_grad)[np.newaxis])
...
def backward_prop(data, labels, params):
    ...
    grad['W2'], grad['b2'], grad_data = backward_linear(W2, b2, flattened, grad_softmax)

    deflattened = np.reshape(grad_data, first_after_relu.shape)
    grad_relu = backward_relu(first_max_pool, deflattened)
    grad_max_pool = backward_max_pool(first_convolution, MAX_POOL_SIZE, MAX_POOL_SIZE, grad_relu)
    grad['W1'], grad['b1'], grad_data2 = backward_convolution(W1, b1, data, grad_max_pool)
